class Airport:
    ...
    def __repr__(self):
        return f"Airport({self.icao}, {self.name}, {self.lat}, {self.long}, {self.reference})"
class Route:
...
airports = [
        ...
        Airport("YMIA", "Mildura ",  -33.9525, 142.2620, "L2"),
        ...
        Airport("YGTH", "Griffith ", -33.9798 ,146.3214, "L2"),
        ...
        Airport("YROM", "Roma ", -26.4509, 148.9142, "L4"),
...
def find_unique_airports(sample_airports, airports):
    """
    Compare two lists of Airport objects and return airports unique to each list based on ICAO code.
    
    Args:
        sample_airports (list): List of Airport objects.
        airports (list): List of Airport objects.
    
    Returns:
        tuple: (unique_to_sample, unique_to_airports)
            - unique_to_sample: List of Airport objects in sample_airports but not in airports.
            - unique_to_airports: List of Airport objects in airports but not in sample_airports.
    """
    # Extract ICAO codes from both lists
    sample_icao_codes = {airport.icao for airport in sample_airports}
    airports_icao_codes = {airport.icao for airport in airports}
    
    # Find ICAO codes unique to each list
    unique_to_sample_icao = sample_icao_codes - airports_icao_codes
    unique_to_airports_icao = airports_icao_codes - sample_icao_codes
    
    # Filter Airport objects based on unique ICAO codes
    unique_to_sample = [airport for airport in sample_airports if airport.icao in unique_to_sample_icao]
    unique_to_airports = [airport for airport in airports if airport.icao in unique_to_airports_icao]
    
    return unique_to_sample, unique_to_airports
# For demonstration, assuming Airport class is defined as:


# Example usage:
unique_to_sample, unique_to_airports = find_unique_airports(sample_airports, airports)

# Print results
print("Airports unique to sample_airports:")
for airport in unique_to_sample:
    print(airport)

print("\nAirports unique to airports:")
for airport in unique_to_airports:
    print(airport)